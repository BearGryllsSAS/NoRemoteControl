...
FriendMessage::FriendMessage(const FriendInfo &info, QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::FriendMessage), friendInfo(info)
{
    ui->setupUi(this);

    //初始化窗口
    QIcon icon(":/Pictures/Resource/Pictures/FriendMessageIcon.png");
    setWindowIcon(icon);
...
{
    ui->AccountTextEdit->setText(info.account);
    ui->NicknameTextEdit->setText(info.name);
    ui->SignatureTextEdit->setText(info.signature);
    ui->GenderTextEdit->setText(info.gender);
    QPixmap avator = base64ToPixmap(info.avator_base64);
    ui->AvatorLabel->setScaledContents(true);
    ui->AvatorLabel->setPixmap(avator);
}
...
void FriendMessage::paintEvent(QPaintEvent *event)//窗口背景
{
    ...
    //加载背景图片
    QPixmap background(":/Pictures/Resource/Pictures/FrinedsMessageBackground.png");
    //确保图片已加载
...
void FriendMessage::on_CloseFriendMessageBtn_clicked()
{
    this->close();
    this->deleteLater();
}
